...
| Relational + spatial (§15.4) | PostgreSQL + PostGIS, Drizzle ORM | **RDS for PostgreSQL 16** (+ RDS Proxy in the generated CDK) | Migrations 0000–0043 in `apps/backend/drizzle` (canonical); `0000_enable_postgis.sql` runs `CREATE EXTENSION IF NOT EXISTS postgis` |
...
- **Migrations:** `apps/backend/drizzle` is **CANONICAL** — 44 files, `0000_enable_postgis` through `0043_driver_name_verified`. `Aws/migrations/` is a **point-in-time snapshot** of the same files and `Aws/db/schema-snapshot.sql` is a pg_dump schema snapshot dated 24 Sep 2026 — use them for review/sizing, never as the deploy source.
