...
| Migrations | 36 SQL files, drizzle-kit format — **canonical location: `apps/backend/drizzle/`** (`Aws/migrations/` is a point-in-time copy) |
...
2. **Ordering.** `apps/backend/drizzle/meta/_journal.json` lists entries `idx` 0–35 with millisecond `when` timestamps; the migrator applies them in that order.
...
| Artifact | What it is |
|---|---|
| `Aws/migrations/` (+ `meta/`) | Point-in-time **copy** of the migration set through idx 35 (`0035_customer_live`), refreshed 23 Sep 2026. **Canonical source is `apps/backend/drizzle/`** — always deploy from there; treat this copy as review material that may lag. |
