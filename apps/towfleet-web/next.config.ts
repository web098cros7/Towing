...
const nextConfig: NextConfig = {
  ...
  // Standalone mode bundles the server + required node_modules into .next/standalone.
  // Required for Amplify Web Compute — the Lambda runs the standalone server directly.
  output: 'standalone',

  // Tell the standalone tracer where the workspace root is so it resolves
  // @towing/* symlinks correctly. Two levels up from apps/towfleet-web.
  // When building inside the turbo-pruned out/ directory this resolves to
  // out/ - the root of the pruned workspace.
